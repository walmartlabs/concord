...
public class AnsibleIT extends AbstractServerIT {
    ...
    @Test(timeout = DEFAULT_TEST_TIMEOUT)
    public void testVerboseLimits() throws Exception {
        ...
        assertLog(".*Too many hosts for verbose logging.*", ab);
        ...
        processApi = new ProcessApi(getApiClient());
        pir = waitForCompletion(processApi, spr.getInstanceId());
        assertEquals("Small inventory with many calls and verbose logging must FAIL",
                ProcessEntry.StatusEnum.FAILED, pir.getStatus());

        // ---

        ab = getLog(pir.getLogFileName());
        assertLog(".*Too much work for verbose logging.*", ab);
