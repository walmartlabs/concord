...
public class LockTaskV2 implements Task {
    ...
    @Inject
    public LockTaskV2(ApiClient apiClient, InstanceId processInstanceId) {
    ...
    public Serializable execute(TaskContext ctx) throws Exception {
        ...
        if (!LockUtils.lock(processInstanceId.getValue(), lockName, scope, apiClient)) {
            suspend(ctx, lockName);
        }

        return null;
    }

    private void suspend(TaskContext ctx, String lockName) {
        ctx.suspend(lockName);
    }
}
