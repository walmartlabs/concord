...
const error: Reducer<string | null, LoginResponse> = (
    state = null,
...
function* onLogin({ username, password, rememberMe, apiKey }: LoginAction) {
    try {
        const response = yield call(apiWhoami, username, password, rememberMe, apiKey);
        yield put({
