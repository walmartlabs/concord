...
public class AgentGitTokenProvider implements GitTokenProvider {
    ...
    public Optional<ActiveAccessToken> getAccessToken(String gitHost, URI repo, @Nullable Secret secret) {
        ...
        return authConfigs.stream()
                ...
                .findFirst()
                .map(auth -> {
                    if (auth instanceof ConcordServer) { // TODO do this as default/fallback? actually, maybe we enrich it with a different api token from system?
    ...
    ActiveAccessToken getTokenFromConcordServer(String gitHost, URI repository) {
