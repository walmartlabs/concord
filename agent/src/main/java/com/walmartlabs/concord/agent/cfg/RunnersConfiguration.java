...
public class RunnersConfiguration {
    ...
    @Inject
    public RunnersConfiguration(Config config) {
        var runtimes = config.getConfig("runners").getObject("runtimes");

        var configs = new HashMap<String, Entry>();
        for (var runtime : runtimes.keySet()) {
            var cfg = Entry.parse(config.getConfig("runners.runtimes." + runtime));
