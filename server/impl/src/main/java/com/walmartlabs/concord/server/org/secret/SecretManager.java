...
public class SecretManager {
    ...
    private byte[] decryptData(SecretEntry secretEntry, String password) {
        byte[] data = getSecretStore(secretEntry.getStoreType()).get(secretEntry.getId());
        if (data == null) {
            throw new IllegalStateException("Can't find the secret's data in the store " + secretEntry.getStoreType() + " : " + secretEntry.getId());
        }

        byte[] pwd = getPwd(password);
        byte[] salt = secretEntry.getSecretSalt();
        return SecretUtils.decrypt(data, pwd, salt, secretEntry.getHashAlgorithm());
    }
    ...
    private UUID create(DSLContext tx,
        ...
        UUID id = secretDao.insert(tx, orgId, name, owner.getId(), type, encryptedByType, storeType, visibility, insertMode, salt, HashAlgorithm.SHA256);
        try {
