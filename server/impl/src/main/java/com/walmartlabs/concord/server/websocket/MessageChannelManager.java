...
public class MessageChannelManager {
    ...
    public <E> Map<MessageChannel, E> getRequests(MessageType requestType) {
        Map<MessageChannel, E> result = new HashMap<>();
        channels.forEach((channelId, channel) -> {
            try {
                channel.getMessage(requestType).ifPresent(msg -> {
                    result.put(channel, (E) msg);
                });
            } catch (Exception e) {
    ...
    @SuppressWarnings("unchecked")
    public <T extends MessageChannel> Optional<T> getChannel(String channelId, Class<T> klass) {
