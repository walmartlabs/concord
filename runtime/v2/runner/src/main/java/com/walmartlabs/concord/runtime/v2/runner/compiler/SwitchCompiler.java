...
public class SwitchCompiler implements StepCompiler<SwitchStep> {
    ...
    public Command compile(CompilerContext context, SwitchStep step) {
        List<KV<String, Command>> caseCommands = new ArrayList<>();
        for (KV<String, List<Step>> kv : step.getCaseSteps()) {
            caseCommands.add(new KV<>(kv.getKey(), compile(context, kv.getValue())));
        }
