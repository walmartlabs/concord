...
class LandingPage extends Component {
  ...
  renderCard(item) {
    ...
    return (
      <Card key={item.id}>
        <Card.Content>
          {item.icon ? (
            <Image floated="right" size="mini" src={`data:image/png;base64, ${item.icon}`} />
          ) : (
            ''
          )}
        ...
        <Card.Content extra>
          <div className="ui two buttons">
            <Button
              basic
              color="green"
              onClick={() => startProcessPopupFn(item.projectName, item.repositoryName)}>
              Start process
            </Button>
          </div>
        </Card.Content>
  ...
  render() {
    ...
    return (
      <div>
        <Header as="h3">
          <RefreshButton loading={loading} onClick={() => this.load()} /> Registered flows
        </Header>

        {(!data || data.length <= 0) && <p>No registered flows found.</p>}
        {data &&
          data.length > 0 && <CardGroup>{data.map((item) => this.renderCard(item))}</CardGroup>}
      </div>
    );
  }
}
