...
const cellFn = (killPopupFn) => (row, key) => {
  ...
  if (key === constants.statusKey) {
    ...
    return (
      <div>
        <Icon name={icon} color={failed ? 'red' : undefined} /> {status}
      </div>
    );
  }
  ...
  if (key === actionsKey) {
    ...
    return canBeKilled && <Button icon="delete" color="red" onClick={() => killPopupFn(id)} />;
  }
...
class QueuePage extends Component {
  ...
  render() {
    ...
    return (
      <div>
        <Header as="h3">
          <RefreshButton loading={loading} onClick={() => this.load()} /> Queue (all available
          organizations)
        </Header>
